    ...
    <div className="fixed inset-0 bg-gradient-to-br from-blue-900 via-purple-900 to-blue-800 flex items-center justify-center z-50">
      <div className="relative w-full max-w-2xl h-64">
        ...
        <motion.div
          ...
          animate={{ x: 200, opacity: [0, 1, 0], scale: [0.5, 1, 0.5] }}
          transition={{ 
            duration: 1, 
            delay: 0.5,
            ease: "easeInOut",
        ...
        {showImpact && (
          ...
          >
            <div className="w-32 h-32 rounded-full border-4 border-white bg-transparent" />
            <div className="absolute inset-0 w-32 h-32 rounded-full border-4 border-yellow-400 opacity-80 animate-ping" />
          </motion.div>
        )}
        ...
        {showResult && (
          ...
          >
            <div className={`text-4xl font-bold ${isCorrect ? 'text-green-400' : 'text-red-400'}`}>
              {isCorrect ? '正解！' : '不正解'}
            </div>
            <div className={`text-lg mt-2 ${isCorrect ? 'text-green-300' : 'text-red-300'}`}>
              {isCorrect ? `効果は抜群だ！` : `効果は今ひとつのようだ…`}
            </div>
